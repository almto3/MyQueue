...
public class TicTacToeGame {
    ...
    public char[] getBoardState() {
        return Arrays.copyOf(mBoard, mBoard.length);
    }

    /**
     * Recreate a TicTacToe game from a pre existing board state.
     * @param boardState Assumed to have 9 spaces and only contains x's, o's, and
     *                   open space.
     */
    public void setBoardState(char[] boardState) {
        for (int i = 0; i < mBoard.length; i++) {
            mBoard[i] = boardState[i];
        }
    }

    public String toStringSimple() {
        StringBuilder sb = new StringBuilder();
        for (char piece : mBoard) {
            sb.append(piece);
        }
        return sb.toString();
    }

    public TicTacToeGame(String board, DifficultyLevel difficulty) {
        this();
        for (int i = 0; i < mBoard.length; i++) {
